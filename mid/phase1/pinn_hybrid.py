...
# STAGE 1: Adam (The "Rough Draft")
# We use Adam first because L-BFGS can be unstable if the guess is too far off.
optimizer_adam = torch.optim.Adam(model.parameters(), lr=0.01)
print("Stage 1: Rough training with Adam...")
...
for epoch in range(2000):
    ...
    loss_phy = physics_loss(model, t_physics)
    loss = loss_pos + loss_vel + loss_phy
...
# STAGE 2: L-BFGS (The "Fine Tuning")
# L-BFGS typically needs fewer iterations but takes more time per step.
...
# The "Closure" function: L-BFGS calls this repeatedly
def closure():
    optimizer_lbfgs.zero_grad()
    
    # Recalculate everything inside the closure
    x_pred_initial = model(t_initial)
    loss_pos = torch.mean((x_pred_initial - x_initial) ** 2)
    ...
    loss_phy = physics_loss(model, t_physics)
    total_loss = loss_pos + loss_vel + loss_phy
    
    # Crucial: Backward pass MUST be inside closure
    total_loss.backward()
    return total_loss

# Run L-BFGS steps
# Note: One L-BFGS "step" performs many internal iterations/evaluations
for i in range(500): # 50 steps is often plenty for L-BFGS
    loss = optimizer_lbfgs.step(closure)
    if i % 10 == 0:
        print(f"L-BFGS Step {i}, Loss: {loss.item():.9f}")
